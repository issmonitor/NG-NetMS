...
<<'HEREDOC_CONFIG'
THIS IS FAKE CONFIG
we do not do any processing of
config that we store to db
so its safe to use fake, but
IF PLUGIN REQUIRES SOME REAL CONFIG
to be parsed REPLACE this text with real device output




GetConfig shoulfd return
this content umodified
and screen signle ' quote
and double "
end some $perl variables
just in case
